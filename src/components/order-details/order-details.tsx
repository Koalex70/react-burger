...
const OrderDetails: FC = () => {
    ...
    if (orderDetailsRequest) {
        return <p>Загрузка...</p>
    } else if (orderDetailsFailed) {
        return <p>Произошла ошибка при получении данных</p>
    }

    return (
        <div className={styles.container}>
            <h3 className={ClassNames(["text text_type_digits-large mb-8", styles.numberShadow])}>{orderDetails?.number}</h3>
            <h4 className={"text text_type_main-medium mb-15"}>{orderDetails?.name}</h4>
            ...
            <p className={"mb-30 text_type_main-default text_color_inactive"}>Дождитесь готовности на орбитальной
                станции</p>
        </div>
    )
}
